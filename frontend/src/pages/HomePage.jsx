...
function HomePage({ user, onLogout }) {
    ...
    const timer = setTimeout(async () => {
      try {
        const response = await fetch(
          `https://api.geoapify.com/v1/geocode/autocomplete?text=${encodeURIComponent(query)}&type=city&format=json&limit=5&apiKey=${geoapifyApiKey}`,
        );
  ...
  const handleStartPlanning = (e) => {
    ...
    if (selectedPlace) {
      ...
      if (selectedPlace.countryCode)
        params.set("country", selectedPlace.countryCode);
  ...
  return (
    <div className="min-h-screen bg-surface text-on-surface font-body-md">
        ...
        <nav className="relative flex justify-between items-center w-full px-6 md:px-16 py-2 max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-2">
            {user ? (
              <>
                <button
                  className="p-2 text-on-surface-variant hover:text-primary transition-colors active:scale-95 duration-150"
        ...
        <section className="relative h-[85vh] flex items-center justify-center overflow-hidden">
          ...
          <div className="relative z-10 w-full max-w-7xl px-6 md:px-16 text-center">
            ...
            <form
              ...
              className="relative bg-white/90 backdrop-blur-xl max-w-3xl mx-auto p-2 rounded-2xl shadow-2xl flex flex-col md:flex-row md:items-stretch gap-2 mb-10"
            >
              ...
              <div className="flex items-center gap-2 px-4 py-3 min-w-0 border-t border-outline-variant md:border-t-0 md:border-l">
                ...
                <input
                  ...
                  className="w-full min-w-0 bg-transparent border-none focus:ring-0 text-base placeholder:text-outline/60 outline-none"
                  value={dateFrom}
              ...
              <button
                type="submit"
                className="shrink-0 bg-primary text-on-primary px-6 py-3 rounded-xl font-label-md hover:bg-primary-container transition-all active:scale-95 whitespace-nowrap"
              >
                Start Planning
              </button>
          ...
          <div className="flex justify-between items-end mb-12">
            <div>
              ...
              <p className="font-body-md text-on-surface-variant mt-1">
                Voted by our community of world travelers.
              </p>
            </div>
            <Link
              to="/planner"
            ...
            {[
              {
                name: "Tokyo, Japan",
                category: "Adventure",
                desc: "From neon skylines to serene temples, experience the future and past in one city.",
                price: "$840",
                img: "https://lh3.googleusercontent.com/aida-public/AB6AXuBbR5zUALkAVvVFlIcx1Cyokg07i9n9yL2nBFEPxhhiHCWMAlfdID1SNuJlCLnpmhDsKkhbOeVSyxDyIZL7x3R1fKhI1GzxW_gicUxESw19di4cEFR3sIUSwUpUAVHa5kHDIuoC3knkweZBze5pmzxZi1lUUWeB0nyjP3WKaNb3HHng8yh6bCTrB6IcJvwECMEmTH1Kn2sqEFRLpD0SgQ-_hOaoVqr1uuU3zp7Um4ehPYC5sAS8eHS-qw4ibNSyWdR1MyCY-kxn6JE",
                catBg: "bg-primary-container",
                catText: "text-on-primary-container",
              },
              {
                name: "Paris, France",
                category: "Culture",
                desc: "The city of lights awaits with art, culinary wonders, and romantic cobblestone streets.",
                price: "$620",
                img: "https://lh3.googleusercontent.com/aida-public/AB6AXuBpIqjdCSRDJ3ocYFtjeFtQJku7KNwfj9_C5mOwYsPRbeqXhnGCrk3KQjlguItxu9uOQQky58Obwy6sMpx6XF6xrg1GQcitNeE4016sTn2rMR3Hjrbqi3G6hRbvTSb3nXvKZiG9eCIr253VcRFYwmp6cdtxVbFTTdGHO8Pyn8EHS-pAyH8BzgAanvlVsGEOBkOv97fS54WbplZL8kflAOktCj1n0f39AF5-gWpN1aUbhRBjUPZCgCeEwWpxUvcLjojIuoDTxCuRBG8",
                catBg: "bg-surface-container-highest",
                catText: "text-on-surface-variant",
              },
              {
                name: "New York, USA",
                category: "Urban",
                desc: "The world's concrete jungle where every corner tells a story of ambition and life.",
                price: "$450",
                img: "https://lh3.googleusercontent.com/aida-public/AB6AXuAk26W5uRbonD4tXjZKditDrqJ9g4g6-nneTGm4oz61d4E9FbhCXVISyLvH6sUmKvWvWJ_SfdgKPf47D7Ftsyk_rPswrUPs2jiyu-E4BZUaFnkF-non8WsEbuWz2NzgWs3GBeNl4WMTx4MRXzC5lnAPRMwStSgMruCSo3D1HuXYv0LVMPTXOe91jaRSoBE1rZ0HBoBn9WZdabaIwJB9eDjwbwv_0ZMbDihl93kcTdBHI_JUFnzo7rUI7gQL2drZzpbsa0OmxQOW-Rk",
                catBg: "bg-tertiary-container",
                catText: "text-on-tertiary-container",
              },
            ].map((dest, idx) => (
              <div
                key={idx}
                className="group relative bg-white rounded-2xl overflow-hidden border border-outline-variant hover:shadow-lg transition-all duration-300"
              >
                ...
                <div className="p-6">
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <span
                      className={`${dest.catBg} ${dest.catText} px-3 py-1 rounded-full font-caption text-caption`}
                    >
                      {dest.category}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      className="w-10 h-10 rounded-full border border-outline-variant flex items-center justify-center hover:bg-surface-container transition-colors group/heart"
                      aria-label={`Save ${dest.name}`}
                    >
                      <Heart
                        size={18}
                        className="transition-colors group-hover/heart:fill-error group-hover/heart:text-error"
                      />
                    </button>
                  </div>
                </div>
        ...
        <section className="py-16 bg-surface-container-low">
          <div className="max-w-7xl mx-auto px-6 md:px-16">
            <div className="text-center mb-12">
              <h2 className="font-headline-lg text-headline-lg text-on-surface">
                Plan Better, Travel More
              </h2>
              <p className="font-body-lg text-on-surface-variant">
                Intelligent tools for the modern explorer.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 grid-rows-2 gap-6 min-h-[600px]">
              <div className="md:col-span-2 md:row-span-2 bg-primary text-on-primary p-10 rounded-2xl flex flex-col justify-end relative overflow-hidden group">
                ...
                <h3 className="font-display-lg text-4xl font-bold mb-4 relative z-10">
                  Smart Itineraries
                </h3>
                <p className="font-body-lg opacity-90 max-w-sm mb-6 relative z-10">
                  Our AI analyzes thousands of routes to suggest the most
                  efficient and scenic journeys based on your interests.
                </p>
                <button className="bg-white text-primary px-8 py-3 rounded-lg font-label-md hover:bg-primary-container hover:text-white transition-colors w-fit relative z-10">
                  Try it now
                </button>
              </div>
              ...
              <div className="md:col-span-2 bg-white border border-outline-variant p-6 rounded-2xl flex gap-6 items-center group hover:border-primary transition-colors">
                ...
                <div>
                  ...
                  <p className="font-body-md text-on-surface-variant">
                    Real-time cost tracking and currency conversion to keep your
                    adventure on track.
                  </p>
                </div>
              </div>
              ...
              <div className="bg-white border border-outline-variant p-6 rounded-2xl group hover:border-primary transition-colors">
                ...
                <p className="font-caption text-on-surface-variant">
                  Share and edit plans with friends in real-time.
                </p>
              </div>
              ...
              <div className="bg-white border border-outline-variant p-6 rounded-2xl group hover:border-primary transition-colors">
                ...
                <p className="font-caption text-on-surface-variant">
                  Take your maps and details anywhere, even without signal.
                </p>
              </div>
            </div>
          </div>
      ...
      <footer className="bg-inverse-surface text-inverse-on-surface py-16">
        <div className="max-w-7xl mx-auto px-6 md:px-16">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12 pb-12 border-b border-white/20">
            <div>
              <h2 className="font-display-lg text-3xl font-extrabold text-inverse-primary mb-3">
                Fly&Go
              </h2>
              <p className="text-on-surface-variant font-body-md">
                Crafting unforgettable journeys since 2026. Your adventure
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  className="flex-1 bg-white/10 border border-white/30 rounded-lg px-3 py-2 text-sm focus:ring-1 focus:ring-inverse-primary placeholder:text-white/30"
                  placeholder="Email address"
                  type="email"
                />
                <button className="bg-inverse-primary text-on-primary-fixed px-4 py-2 rounded-lg font-label-md hover:bg-white transition-colors">
                  Join
                </button>
              </div>
            </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 text-on-surface-variant font-caption">
            <p>© 2026 Fly&Go | All rights reserved.</p>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-3 py-2 pb-4 bg-surface border-t border-outline-variant shadow-md rounded-t-2xl">
        <Link
          to="/planner"
          className="flex flex-col items-center justify-center bg-primary-container text-on-primary-container rounded-full px-4 py-1 transition-all"
        >
          <Compass size={22} />
        ...
        <Link
          to="/budget"
          className="flex flex-col items-center justify-center text-on-surface-variant hover:bg-surface-variant transition-all"
        >
          <Map size={22} />
