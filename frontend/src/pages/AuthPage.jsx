...
function AppleIcon() {
  return (
    <svg
      className="w-5 h-5 fill-on-surface"
      viewBox="0 0 24 24"
      aria-hidden="true"
    >
...
function AuthPage({ user, onLogin, navigate }) {
  ...
  const socialLogin = () =>
    setError(
      "Social sign-in isn’t available yet — please use your email and password.",
    );
    ...
    <main className="flex flex-col md:flex-row min-h-screen w-full overflow-hidden">
      ...
      <section className="hidden md:flex relative md:w-1/2 lg:w-7/12 items-center justify-center overflow-hidden">
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(180deg, rgba(11,28,48,0.4) 0%, rgba(11,28,48,0) 50%, rgba(11,28,48,0.7) 100%)",
          }}
        />
        ...
        <div className="relative z-10 p-16 max-w-xl text-white">
          <h1 className="font-display-lg text-display-lg mb-6 leading-tight text-white">
            Start your next <span className="text-blue-400">extraordinary</span>{" "}
            journey.
          </h1>
          <p className="font-body-lg text-body-lg text-white/90 mb-10">
            Fly&Go helps you organize chaotic ideas into structured, trustworthy
            itineraries with high-end tools for the modern traveler.
          </p>
          <div className="flex items-center gap-3">
            <div className="flex -space-x-3">
              <img
                ...
                alt="Traveler"
                src="https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?q=80&w=400&auto=format&fit=facearea&facepad=2"
              />
              <img
                className="w-10 h-10 rounded-full border-2 border-white object-cover"
                alt="Traveler"
                src="https://images.unsplash.com/photo-1544005313-94ddf0286df2?q=80&w=400&auto=format&fit=facearea&facepad=2"
              />
              <div className="w-10 h-10 rounded-full border-2 border-white bg-primary text-white flex items-center justify-center font-label-md text-[10px]">
                +12k
              </div>
            </div>
            <span className="font-label-md text-label-md text-white">
              Joined by explorers worldwide
            </span>
          </div>
        </div>

        {/* Branding overlay (Desktop) */}
      ...
      <section className="w-full md:w-1/2 lg:w-5/12 bg-surface flex flex-col items-center justify-center px-4 md:px-16 py-16 relative">
        {/* Mobile branding - ARRUMADO AQUI */}
        <div className="md:hidden w-full mb-10 flex items-center justify-center">
          <img
            src="/images/flyandgo.png"
            alt="Logo Fly&Go"
            className="h-26 w-auto object-contain" // h-16 para mobile fica bom
          />
        </div>
        ...
        <div className="w-full max-w-md">
          <header className="mb-10">
            ...
            <p className="font-body-md text-body-md text-on-surface-variant">
              {isLogin
                ? "Log in to your account to continue planning."
                : "Sign up to start planning your next adventure."}
            </p>
          </header>

          {error ? (
            <div className="mb-6 rounded-lg bg-error-container px-4 py-3 text-label-md text-on-error-container">
              {error}
            </div>
          ) : null}
          ...
          <div className="flex flex-col gap-2 mb-10">
            <button
              ...
              className="w-full flex items-center justify-center gap-3 py-2.5 px-6 border border-outline-variant rounded-lg bg-surface-container-lowest hover:bg-surface-container-low transition-colors duration-200"
            >
              <GoogleIcon />
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-2.5 bg-primary text-white font-label-md text-label-md rounded-lg shadow-sm hover:bg-primary-container active:scale-[0.98] transition-all duration-200 flex items-center justify-center gap-2 group disabled:cursor-not-allowed disabled:opacity-60"
            >
              {loading
                ? "Please wait…"
                : isLogin
          ...
          <footer className="mt-16 text-center">
            ...
            <div className="mt-10 flex flex-wrap justify-center gap-6 text-outline">
              <a
                className="font-caption text-caption hover:text-on-surface transition-colors"
                href="#"
              >
                Privacy Policy
              </a>
